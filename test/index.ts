...
async function main() {
  ...
  const totalNfts: number = 9 // set 1500 later
  ...
  for (let i = startTokenID; i <= maxTokenID; i += mintSize) {
    ...
    const ids: number[] = Array(currentMintSize)
      .fill(0)
      .map((index) => index + i)
    // array containing amount to mint for each tokenId, 1 in case of NFT's
    const amounts: number[] = Array(currentMintSize).fill(1)
    ...
    await coolGirlNFT.mintBatch(signer.address, ids, amounts, '')
